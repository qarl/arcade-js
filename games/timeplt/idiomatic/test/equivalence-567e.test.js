...
/**
 * loc_567e — memory-equivalent to the frozen oracle at ROM 0x567E.
 *
 * WHAT IT IS. Two instructions: load one byte of the program image into A, then transfer to the
 * shared request body at ROM 0x5617, WHICH IS ALREADY DECOMPILED — so the rewrite calls it
 ...
 * permission it asks under, and this gate is built to see both: the permission is the play-active flag OR the demo-sound switch,
 ...
 *   machine state captured at its tail at ROM 0x5617 instead — a genuine in-play state with a
...
test("CRAFTED ENTRY: from a real capture taken at the tail: loc_567e == oracle on RAM", { skip }, () => {
  const entry = entryState();
  const a = entry.clone();
  const b = entry.clone();
  oracle(a);
...
test("IT READS THE IMAGE: the requested code follows a poked source byte", { skip }, () => {
  ...
  withPokedImage(entry, SOUND_CODE_CELL, POKED_CODE, () => {
    ...
    const d = craftedDiff(loc_567e, 0xff, 0x01, 3);
...
test("GATE CROSS: both permission cells swept, including the drop branch", { skip }, () => {
  for (const play of PLAY_VALUES) {
    for (const demo of DEMO_VALUES) {
      const d = craftedDiff(loc_567e, play, demo, 3);
...
test("EXHAUSTIVE over the queue length", { skip }, () => {
  for (let length = 0; length < 256; length++) {
    const d = craftedDiff(loc_567e, 0xff, 0x01, length);
