...
/**
 * Equivalence-harness tests for guard_3131 (ROM 0x3131) -- one of the four
 ...
 * NORMALLY when that is < 7 (seven frames in eight), else performs the CALLER-SKIP
 ...
 * guard_3131 IS reachable from boot. Its live reach is real and translated: the
 * gameplay/attract cascade loc_197a (dispatched in the vblank NMI) calls
 ...
 * [3110,3110,311b,3126,3126,3131] -- selecting guard_3131 at DIFFICULTY 5. In pure
 * attract DIFFICULTY is 1 (-> guard_3110), so an IDENTICAL-BOTH-SIDES poke holds
 * 0x6380 = 5 across the attract-demo window (Karl's sanctioned "poke the board
 * state to reach a state for validation"), which dispatches guard_3131 live 56x
 * from frame ~586 -- and BOTH branches fire naturally under it (49 normal-return +
 * 7 caller-skip, as FRAME cycles 0..7). The poke is threaded via a custom
 * makeMachine factory (m.pokes) driving the game-agnostic CORE equivalence engine
 * (like equivalence-3126 / 16a3); the DK harness.js wrapper bakes assets but not
 * the timed pokes. Applied identically to baseline and optimized, so the only
 * difference the gate sees is translated vs optimized.
 *
 ...
 * downstream at SPIN_COUNT (0x6019) -- and the polarity TEETH below is caught live.
 * The per-branch totals are ALSO pinned in isolation (synth tests).
 *
 ...
 *
 * THE CYCLE FINDING. guard_3131 is ATOMIC by construction -- a pure LEAF (no
 ...
 * polarity, wrong SP, wrong polarity, wrong cycle total, wrong override -- each
 * CAUGHT).
 *
 * Run: node --test
 */
...
import { guard_3131 as translated_3131 } from "../../translated/state0.js";
import { guard_3131 as optimized_3131 } from "../guard_3131.js";
...
const TARGET = 0x3131;
const FRAMES = 640; // guard_3131 dispatches ~frame 586..619 under the poke; run past it so a wrong total surfaces at SPIN_COUNT

// The predicate straight from the ROM bytes: normal-return (true) when the low 3
// bits of FRAME (0x601A) are < 7; caller-skip (false) when they equal 7.
const wantNormal = (v) => (v & 0x07) < 0x07;
...
// [3110,3110,311b,3126,3126,3131]; value 5 -> index 5 -> guard_3131. Held across
// the attract-demo window so guard_3131 dispatches live every frame the cascade runs.
const DIFFICULTY = 0x6380;
const FORCE_3131_POKE = [{ addr: DIFFICULTY, val: 0x05, frame: 580, dur: 45 }];
...
const makeMachine = (overrides) => {
  ...
  m.pokes = FORCE_3131_POKE.map((p) => ({ ...p }));
  return m;
};

/**
 * Synthesise the entry the family is reached in (boot.test.js layout, matching
 * equivalence-3126): SP = 0x6C00 with two return frames beneath it -- the caller's
 * CALLER first (0xCAFE), then the caller's own return (0xBEEF) -- and FRAME poked
 * to v. A normal return pops 0xBEEF (caller); the caller-skip discards it and pops
 * 0xCAFE (caller's caller). Fresh machine, so `cycles` starts at 0 and after the
 * routine equals the branch total.
 */
function entry(v) {
...
/** The documented POLARITY TRAP: compares against 0x06 instead of 0x07 -- flips
 *  the boolean, PC and F for the input FRAME&7 == 6 (normal -> caller-skip). */
function broken_polarity(m) {
  const { regs, mem } = m;
  regs.a = mem.read8(0x601a);
  regs.and(0x07);
  regs.cp(0x06); // BUG: wrong compare, one below 0x07
  if (regs.fM) {
    m.ret(38);
    return true;
  }
...
test("EQUAL (whole-machine): idiomatic optimized guard_3131 matches translated every frame", () => {
  const r = coreWholeMachineEquivalence(makeMachine, FRAMES, new Map([[TARGET, optimized_3131]]));
  ...
  console.log(
    `  EQUAL/whole: ${r.framesCompared} frames identical, ` +
      `override fired ${r.invocations.get(TARGET)}x (both branches exercised under the DIFFICULTY=5 poke)`,
  );
});
...
test("DISPATCH: the override fires through the live rst-0x28 dispatcher and returns the oracle's value", () => {
  ...
  const oracle = translated_3131(entry(0x06));
...
test("EQUAL (unit, synthesised): normal-return branch (0x601A=6, (v&7)<7) matches in RAM + regs + PC + cycles", () => {
  const d = synthDiff(0x06, translated_3131, optimized_3131);
...
test("EQUAL (unit, synthesised): caller-skip branch (0x601A=7, (v&7)==7) matches in RAM + regs + PC + cycles", () => {
  const d = synthDiff(0x07, translated_3131, optimized_3131);
  assert.equal(d.ram, null, d.ram ? `RAM diff at 0x${d.ram.addr.toString(16)}` : "");
  assert.equal(d.regs, null, d.regs ? `reg diff at ${d.regs.reg}` : "");
  assert.equal(d.pc, null, "pc must match");
...
test("EQUAL (exhaustive): every 0x601A in 0x00..0x0F matches oracle in RAM + regs + PC + boolean + SP + cycle total", () => {
  ...
  for (let v = 0; v <= 0x0f; v++) {
    const d = synthDiff(v, translated_3131, optimized_3131);
...
test("TEETH (whole-machine): a wrong polarity (cp 0x06) is CAUGHT live and NOT-EQUAL", () => {
...
test("TEETH (unit): a wrong outgoing SP on the skip branch is CAUGHT and names SP", () => {
  const d = synthDiff(0x07, translated_3131, broken_sp);
...
test("TEETH (unit): the polarity trap (cp 0x06 instead of 0x07) is CAUGHT — flips boolean + flags", () => {
  const d = synthDiff(0x06, translated_3131, broken_polarity);
  assert.notEqual(d.retA, d.retB, "polarity change must flip the boolean at 0x601A=6");
...
test("TEETH (unit): a 1-cycle-wrong collapsed total (37 not 38) is CAUGHT — the cycle assertion has teeth", () => {
  const d = synthDiff(0x06, translated_3131, broken_cycles);
