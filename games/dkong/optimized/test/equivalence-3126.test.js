...
/**
 * Equivalence-harness tests for guard_3126 (ROM 0x3126) -- one of the four
 * 0x3110 GUARD FAMILY members, a SKIP-CAPABLE rst-0x28 dispatch target on the
 * NMI / sub_30fa path. It masks FRAME (0x601A) to its low 2 bits and returns
 * NORMALLY when that is < 3 (three frames in four), else performs the CALLER-SKIP
 ...
 * guard_3126 IS reachable from boot -- correcting the sibling guard_3131's stale
 * "unreachable" note. Its live reach is real and translated: the gameplay/attract
 * cascade loc_197a (dispatched in the vblank NMI) calls entry_30ed, which calls
 * sub_30fa, which reads DIFFICULTY (0x6380), clamps it to [0,5], and rst-0x28
 * dispatches the guard-family table [3110,3110,311b,3126,3126,3131] -- selecting
 * guard_3126 at DIFFICULTY 3 or 4. In pure attract DIFFICULTY is 1 (-> guard_3110),
 * so an IDENTICAL-BOTH-SIDES poke holds 0x6380 = 3 across the attract-demo window
 * (Karl's sanctioned "poke the board state to reach a state for validation"),
 * which dispatches guard_3126 live 56x from frame ~586 -- and BOTH branches fire
 * naturally under it (42 normal-return + 14 caller-skip, as FRAME cycles 0..3). The
 * poke is threaded via a custom makeMachine factory (m.pokes) driving the game-
 * agnostic CORE equivalence engine (like equivalence-28b0 / 16a3); the DK harness.js
 ...
 *
 * THE CYCLE FINDING. guard_3126 is ATOMIC by construction -- a pure LEAF (no
...
import { guard_3126 as translated_3126 } from "../../translated/state0.js";
import { guard_3126 as optimized_3126 } from "../guard_3126.js";
...
const TARGET = 0x3126;
const FRAMES = 640; // guard_3126 dispatches ~frame 586..619 under the poke; run past it so a wrong total surfaces at SPIN_COUNT

// The predicate straight from the ROM bytes: normal-return (true) when the low 2
// bits of FRAME (0x601A) are < 3; caller-skip (false) when they equal 3.
const wantNormal = (v) => (v & 0x03) < 0x03;
...
// [3110,3110,311b,3126,3126,3131]; value 3 -> index 3 -> guard_3126. Held across
// the attract-demo window so guard_3126 dispatches live every frame the cascade runs.
const DIFFICULTY = 0x6380;
const FORCE_3126_POKE = [{ addr: DIFFICULTY, val: 0x03, frame: 580, dur: 45 }];
...
const makeMachine = (overrides) => {
  ...
  m.pokes = FORCE_3126_POKE.map((p) => ({ ...p }));
  return m;
};

/**
 * Synthesise the entry the family is reached in (boot.test.js layout, matching
 * equivalence-3131): SP = 0x6C00 with two return frames beneath it -- the caller's
...
/** The documented POLARITY TRAP: compares against 0x02 instead of 0x03 -- flips
 *  the boolean, PC and F for the input FRAME&3 == 2 (normal -> caller-skip). */
function broken_polarity(m) {
  const { regs, mem } = m;
  regs.a = mem.read8(0x601a);
  regs.and(0x03);
  regs.cp(0x02); // BUG: wrong compare, one below 0x03
  if (regs.fM) {
    m.ret(38);
    return true;
  }
...
test("EQUAL (whole-machine): idiomatic optimized guard_3126 matches translated every frame", () => {
  const r = coreWholeMachineEquivalence(makeMachine, FRAMES, new Map([[TARGET, optimized_3126]]));
  ...
  console.log(
    `  EQUAL/whole: ${r.framesCompared} frames identical, ` +
      `override fired ${r.invocations.get(TARGET)}x (both branches exercised under the DIFFICULTY=3 poke)`,
  );
});
...
test("DISPATCH: the override fires through the live rst-0x28 dispatcher and returns the oracle's value", () => {
  ...
  const oracle = translated_3126(entry(0x02));
...
test("EQUAL (unit, synthesised): normal-return branch (0x601A=2, (v&3)<3) matches in RAM + regs + PC + cycles", () => {
  const d = synthDiff(0x02, translated_3126, optimized_3126);
...
test("EQUAL (unit, synthesised): caller-skip branch (0x601A=3, (v&3)==3) matches in RAM + regs + PC + cycles", () => {
  const d = synthDiff(0x03, translated_3126, optimized_3126);
  assert.equal(d.ram, null, d.ram ? `RAM diff at 0x${d.ram.addr.toString(16)}` : "");
  assert.equal(d.regs, null, d.regs ? `reg diff at ${d.regs.reg}` : "");
  assert.equal(d.pc, null, "pc must match");
...
test("EQUAL (exhaustive): every 0x601A in 0x00..0x0F matches oracle in RAM + regs + PC + boolean + SP + cycle total", () => {
  ...
  for (let v = 0; v <= 0x0f; v++) {
    const d = synthDiff(v, translated_3126, optimized_3126);
...
test("TEETH (whole-machine): a wrong polarity (cp 0x02) is CAUGHT live and NOT-EQUAL", () => {
...
test("TEETH (unit): a wrong outgoing SP on the skip branch is CAUGHT and names SP", () => {
  const d = synthDiff(0x03, translated_3126, broken_sp);
...
test("TEETH (unit): the polarity trap (cp 0x02 instead of 0x03) is CAUGHT — flips boolean + flags", () => {
  const d = synthDiff(0x02, translated_3126, broken_polarity);
  assert.notEqual(d.retA, d.retB, "polarity change must flip the boolean at 0x601A=2");
...
test("TEETH (unit): a 1-cycle-wrong collapsed total (37 not 38) is CAUGHT — the cycle assertion has teeth", () => {
  const d = synthDiff(0x02, translated_3126, broken_cycles);
